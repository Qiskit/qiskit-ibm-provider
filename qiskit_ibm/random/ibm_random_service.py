...
"""IBM Quantum random number service."""
...
class IBMRandomService:
    """Random number services for an IBM Quantum account provider.
    ...
    :class:`~qiskit_ibm.IBMProvider` class.
    ...
    def get_service(self, name: str) -> BaseRandomService:
        ...
        Raises:
            IBMError: If the service cannot be found.
        """
        self._discover_services()
        service = self._services.get(name, None)
        if service is None:
            raise IBMError('No service with the name {} can be found.'.format(name))
