...
class LiveDataVisualization:
    ...
    def get_livedata_jobs(self) -> list:
        ...
        if provider is not None and not isinstance(provider.backend, FakeBackend):
            ...
            livedata_jobs = [
                job for job in total_jobs if getattr(job, "liveDataEnabled", True)
            ]
    ...
    def create_title(self, title, extra_space: bool = False) -> widgets.HTML:
        ...
        content = (
            f"<h5 style='margin-top: {margin};margin-bottom: 8px;'><b>{title}</b></h5>"
        )
    ...
    async def init_websockets(self) -> None:
        ...
        try:
            ...
            ws_connection = WebSocketApp(
                uri, on_open=self.ws_on_open, on_message=self.ws_on_message
            )
            ...
            if self.ws_connection == this_ws:
                logger.debug(
                    "🤖 Trying to reconnect ws@job_id #%s...", self.selected_job["id"]
                )
            ...
            if len(self.jobs) > 0:
                ...
                if self.jobs_combo.value != "":
                    if previous_len == new_len:
                        ...
                        logger.debug(
                            "changing job to the new received: %s", self.job_ids[0]
                        )
